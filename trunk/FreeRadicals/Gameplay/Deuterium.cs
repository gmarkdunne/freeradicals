...
namespace FreeRadicals.Gameplay
{
    ...
    /// </summary>
    class Deuterium : Actor, IBaseAgent
    {
        #region Constants
        ...
        /// </summary>
        static readonly Color[] H2OColors = 
        { 
            Color.Yellow, Color.Red
        };
        static readonly Color[] CH2Colors = 
        { 
        ...
        /// </summary>
        private VectorPolygon hydrogenPolygon = null;
            ...
            : base(world)
        {
            ...
            this.hydrogenPolygon = VectorPolygon.CreateCircle(new Vector2(10f, 0), radius, 100);
        ...
        public override void Draw(float elapsedTime, LineBatch lineBatch)
        {
            ...
            if (hydrogenPolygon != null)
            {
                ...
                Matrix world = rotationMatrix *
                    Matrix.CreateTranslation(position.X, position.Y, 0f);
                // transform the polygon
                hydrogenPolygon.Transform(world);
                // draw the polygon
                lineBatch.DrawPolygon(hydrogenPolygon, color);
                ...
                if (useMotionBlur && velocity.LengthSquared() > 1024f)
                {
                    ...
                    for (int i = 1; i < speed / 16; ++i)
                    {
                        ...
                        Matrix blurWorld = rotationMatrix *
                            Matrix.CreateTranslation(blurPosition.X, blurPosition.Y, 0);
                        // transform the polygon to the "blur" location
                        hydrogenPolygon.Transform(blurWorld);
                        // calculate the alpha of the "blur" location
                        byte alpha = (byte)(160 / (i + 1));
                        if (alpha < 1)
                            break;
                        // draw the "blur" polygon
                        lineBatch.DrawPolygon(hydrogenPolygon,
                            new Color(color.R, color.G, color.B, alpha));
                    }
                }
            }
        ...
        public override bool Touch(Actor target)
        {
            ...
            // if the Deuterium hits an oxygen, Bond them to make H2O
            if ((target is Oxygen) == true)
            {
                ...
                world.ParticleSystems.Add(new ParticleSystem(newPosition,
                    newDirection, 36, 64f, 128f, 2f, 0.05f, H2OColors));
                this.world.AudioManager.PlayCue("asteroidTouch");
            }
            ...
            if ((target is Carbon) == true)
            {
                ...
                world.ParticleSystems.Add(new ParticleSystem(newPosition,
                    newDirection, 36, 64f, 128f, 2f, 0.05f, CH2Colors));
                this.world.AudioManager.PlayCue("asteroidTouch");
            }
            ...
            //    world.UnbondDeuterium(newPosition, newVelocity, newDirection);
            //    world.ParticleSystems.Add(new ParticleSystem(newPosition,
